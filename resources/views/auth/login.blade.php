...
<body class="bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
    ...
    <main class="mt-20 max-w-md mx-auto px-6 py-8">
        <div
            ...
            <p class="text-slate-600 dark:text-slate-400 mb-6">Masuk ke akun Anda untuk berinteraksi</p>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="remember" id="remember"
                        class="w-4 h-4 rounded border-slate-300 dark:border-slate-600 dark:bg-slate-800 cursor-pointer">
                    <label for="remember" class="ml-2 text-sm cursor-pointer">Ingat saya</label>
                </div>
            ...
            <p class="mt-6 text-center text-sm text-slate-600 dark:text-slate-400">
                Belum punya akun? <a href="{{ route('register') }}" class="font-semibold hover:underline">Daftar di
                    sini</a>
            </p>
        </div>
    </main>
