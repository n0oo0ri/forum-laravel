...
<head>
    ...
    <title>Edit Postingan - Forum</title>
...
<body class="bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
    ...
    <main class="max-w-3xl mx-auto px-6 py-8">
        <div class="mb-6">
            <h1 class="text-3xl font-bold mb-2">Edit Postingan</h1>
            <p class="text-slate-600 dark:text-slate-400">Perbarui postingan Anda</p>
        </div>
        ...
        <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                @if ($post->media_path)
                    <div class="mb-4 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
                        <p class="text-sm text-slate-600 dark:text-slate-400 mb-3">Media Saat Ini:</p>
                        @if ($post->media_type === 'image')
                            <img src="{{ Storage::url($post->media_path) }}" alt="{{ $post->title }}"
                                class="max-h-48 rounded-lg mx-auto">
                        @elseif ($post->media_type === 'video')
                            <video class="max-h-48 mx-auto rounded-lg" controls>
                                <source src="{{ Storage::url($post->media_path) }}" type="video/mp4">
                            </video>
                        @endif
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-2">Unggah file baru untuk mengganti
                            media</p>
                    </div>
                @endif
                ...
                <div id="media-drop-zone"
                    ...
                    <div id="media-placeholder" class="pointer-events-none">
                        ...
                        <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Klik atau seret foto/video di sini
                        </p>
                        <p class="text-xs text-slate-500 dark:text-slate-500 mt-1">Maksimal 50MB (JPEG, PNG, GIF, MP4,
                            WebM)</p>
                    </div>
                    ...
                    <div id="media-preview" class="hidden mt-4">
                        ...
                        <div id="media-filename" class="text-sm text-slate-600 dark:text-slate-400 mt-2"></div>
